...
export type StreamImage = {
  ...
  alt?: string;
  /** Optional label to display below the image */
  label?: string;
};
          ...
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              return (
                <div
                  key={`${name}-${i}`}
                  className={cn(card, "absolute overflow-visible")}
                  style={{
                    left: "50%",
                    top: `${axis}%`,
                ...
                >
                  <div className="relative h-full w-full overflow-hidden" style={{ borderRadius: `${p.cardRadius}cqw` }}>
                    {img ? (
                      <img
                        src={img.src}
                        alt={img.alt ?? ""}
